...
###############################################################################
from src.view.util.common.ed_event import StatusEvent, edEVT_STATUS
from src.view.util.common.ed_glob import SB_BUFF
from src.view.util.common.ed_vim import EditraCommander, Parse
...
class KeyHandler(object):
    ...
    def ClearMode(self):
        ...
        evt = StatusEvent(edEVT_STATUS, self.stc.Id,
                                   '', SB_BUFF)
...
class ViKeyHandler(KeyHandler):
    ...
    def __init__(self, stc, use_normal_default=False):
        ...
        self.last = u''
        self.last_find = u''
        self.commander = EditraCommander(self)
        self.buffer = u''
    ...
    def _SetMode(self, newmode, msg):
        ...
        self.mode = newmode
        # Update status bar
        evt = StatusEvent(edEVT_STATUS, self.stc.GetId(),
                                   msg, SB_BUFF)
    ...
    def _ProcessKey(self, key_code):
        ...
        if self.IsNormalMode() or self.IsVisualMode():
            self.buffer += char
            if Parse(self.buffer, self.commander):
