...
import re
from src.view.util.common.eclutil import HexToRGB
import os
from src.view.util.common.util import GetFileReader
...
#             item.size=10
    ...
    def GetStyleSet(self):
        ...
        """
        return StyleManager.STYLES.get(self.style_set, DEF_STYLE_DICT)
...
#                 logger.debug("[ed_style][err] Failed to parse style data for %s:" % style_sheet)
                return False
...
#                 self.SetStyles('default', DEF_STYLE_DICT)
            ...
            logger.debug("[ed_style][info] Using cached style data")
            return True
...
#        style_data = style_data.replace(u" ", u"") # support old style
        ...
        for branch in ttree:
            ...
            if len(branch) != 2:
                logger.debug("[ed_style][err] There was an error parsing "
                         "the syntax data from " + self.style_set)
                logger.debug("[ed_style][err] Missing a { or } in Def: " + repr(branch[0]))
                ttree.remove(branch)
                continue
        ...
        for branch in style_tree:
            ...
            for leaf in branch[1]:
                ...
                if len(leaf) != 2:
                    logger.debug("[ed_style][err] Missing a : or ; in the "
                             "declaration of %s" % tag)
                elif leaf[0] not in STY_ATTRIBUTES:
                    logger.debug(("[ed_style][warn] Unknown style attribute: %s"
    ...
    def SetStyles(self, name, style_dict, nomerge=False):
        ...
        if nomerge:
            self.style_set = name
            StyleManager.STYLES[name] = self.PackStyleSet(style_dict)
            return True
        ...
        if isinstance(style_dict, dict):
            ...
            for style in style_dict.values():
                if not isinstance(style, StyleItem):
                    logger.debug("[ed_style][err] Invalid data in style dictionary")
            ...
            if dstyle is None:
                logger.debug("[ed_style][warn] default_style is undefined")
            ...
            logger.debug("[ed_style][err] SetStyles expects a " \
