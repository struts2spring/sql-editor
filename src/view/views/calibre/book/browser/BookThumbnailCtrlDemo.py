...
class ThumbnailCtrlDemoPanel(wx.Panel):
...
#         return menu_bar
    ...
    def OnSetDirectory(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            logger.info("OnSetDirectory: directory changed to: %s\n"%dlg.GetPath())
    ...
    def OnChangeOutline(self, event): # wxGlade: MyFrame.<event_handler>
        ...
        logger.info("OnChangeOutline: Outline changed to: %s\n"%self.thumbstyles[pos])
        event.Skip()
    ...
    def OnHighlight(self, event): # wxGlade: MyFrame.<event_handler>
        ...
        if self.highlight.GetValue() == 1:
            ...
            logger.info("OnHighlight: Highlight thumbs on pointing\n")
        else:
            self.TC.SetHighlightPointed(False)
            logger.info("OnHighlight: Don't Highlight thumbs on pointing\n")
    ...
    def OnShowFiles(self, event): # wxGlade: MyFrame.<event_handler>
        ...
        if self.showfiles.GetValue() == 1:
            ...
            logger.info("OnShowFiles: Thumbs file names shown\n")
        else:
            self.TC.ShowFileNames(False)
            logger.info("OnShowFiles: Thumbs file names not shown\n")
    ...
    def OnEnableDragging(self, event):
        ...
        if self.enabledragging.GetValue() == 1:
            ...
            logger.info("OnEnableDragging: Thumbs drag and drop enabled\n")
        else:
            self.TC.EnableDragging(False)
            logger.info("OnEnableDragging: Thumbs drag and drop disabled\n")
    ...
    def OnSetPopup(self, event): # wxGlade: MyFrame.<event_handler>
        ...
        if self.setpopup.GetValue() == 1:
            ...
            logger.info("OnSetPopup: Popups enabled on thumbs\n")
        else:
            self.TC.SetPopupMenu(None)
            logger.info("OnSetPopup: Popups disabled on thumbs\n")
    ...
    def OnSetGlobalPopup(self, event):
        ...
        if self.setgpopup.GetValue() == 1:
            ...
            logger.info("OnSetGlobalPopup: Popups enabled globally (no selection needed)\n")
        else:
            self.TC.SetGlobalPopupMenu(None)
            logger.info("OnSetGlobalPopup: Popups disabled globally (no selection needed)\n")
    ...
    def OnShowComboBox(self, event):
        ...
        if self.showcombo.GetValue() == 1:
            logger.info("OnShowComboBox: Directory comboBox shown\n")
            self.TC.ShowComboBox(True)
        else:
            logger.info("OnShowComboBox: Directory comboBox hidden\n")
    ...
    def OnEnableToolTips(self, event):
        ...
        if self.enabletooltip.GetValue() == 1:
            logger.info("OnEnableToolTips: File information on tooltips enabled\n")
            self.TC.EnableToolTips(True)
        else:
            logger.info("OnEnableToolTips: File information on tooltips disabled\n")
    ...
    def OnSelChanged(self, event):
        ...
        logger.info("OnSelChanged: Thumb selected: %s\n"%str(self.TC.GetSelection()))
        event.Skip()


    def OnPointed(self, event):

        logger.info("OnPointed: Thumb pointed: %s\n"%self.TC.GetPointed())
        event.Skip()


    def OnDClick(self, event):

        logger.info("OnDClick: Thumb double-clicked: %s\n"%self.TC.GetSelection())
        event.Skip()
    ...
    def OnSetFont(self, event): # wxGlade: MyFrame.<event_handler>
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.TC.Refresh()
            logger.info("OnSetFont: Caption font changed\n")
...
#         item.SetBitmap(bmp)
    ...
    def OnPopupOne(self, event):
        logger.info("OnPopupMenu: Popup One\n")


    def OnPopupTwo(self, event):
        logger.info("OnPopupMenu: Popup Two\n")


    def OnPopupThree(self, event):
        logger.info("OnPopupMenu: Popup Three\n")


    def OnPopupFour(self, event):
        logger.info("OnPopupMenu: Popup Four\n")


    def OnPopupFive(self, event):
        logger.info("OnPopupMenu: Popup Five\n")


    def OnPopupSix(self, event):
        logger.info("OnPopupMenu: Popup Six\n")


    def OnPopupSeven(self, event):
        logger.info("OnPopupMenu: Popup Seven\n")


    def OnPopupEight(self, event):
        logger.info("OnPopupMenu: Popup Eight\n")


    def OnPopupNine(self, event):
        logger.info("OnPopupMenu: Popup Nine\n")


    def OnPopupTen(self, event):

        items = self.TC.GetItemCount()
        logger.info("Items", items, type(items))

        for ii in range(items):
            self.TC.SetSelection(ii)

        logger.info("OnGlobalPopupMenu: all thumbs selected\n")
    ...
    def OnPopupEleven(self, event):
        ...
        logger.info("OnGlobalPopupMenu: say hello message...\n")
    ...
    def OnPopupTwelve(self, event):
        ...
        items = self.TC.GetItemCount()
        logger.info("OnGlobalPopupMenu: number of thumbs: %d\n"%items)
...
#     app.MainLoop()
    ...
    frame = wx.Frame(None)
    try:
        panel = ThumbnailCtrlDemoPanel(frame)
