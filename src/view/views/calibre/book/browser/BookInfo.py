...
#         imgFilePath=os.path.join(relevant_path,imgFileName[0] )
        imgFilePath = self.getImageFilePath()
            
#         img2 =  imgFilePath=os.path.join(relevant_path,imgFileName[1] )
        logger.debug(f'PropertyPhotoPanel GetSize {self.GetSize()}')
...
#         self.rt = wx.richtext.RichTextCtrl(self, style=wx.VSCROLL | wx.HSCROLL | wx.NO_BORDER)
        self.rt = RichTextPanel(self, self.currentBook)
...
#         self.SetAutoLayout(True)
    def getImageFilePath(self):
        os.chdir(self.currentBook.bookPath)
        imgFilePath = os.path.join(self.currentBook.bookPath, self.currentBook.bookImgName)
        if not os.path.exists(imgFilePath):
            directory = '.'
            pattern = re.compile(r"\-(\d*)\.jpg$")
...
#                     print(m.groups())
    ...
    def scale_bitmap(self, width=None, height=None):
        bitmap = wx.Image(self.getImageFilePath(), wx.BITMAP_TYPE_JPEG).ConvertToBitmap()
