...
from src.view.util.common.FileObjectWrapper import EVT_FILE_LOAD, \
    FL_STATE_READING, FL_STATE_END, FL_STATE_START, FL_STATE_ABORTED
from src.view.util.common.eclutil import Freezer
from src.view.util.common.ed_glob import *
from src.view.util.common.ed_keyh import KeyHandler, ViKeyHandler
from src.view.util.common.ed_marker import Bookmark
...
#             self.SetDropTarget(DropTargetFT(self, None, parent.OnDrop))
        ...
        self._config = dict(autocomp=True,
                            autoindent=True,
                            brackethl=True,
                            folding=True,
                            highlight=True,
...
#         exec code in self.__dict__ # Inject new code into this namespace
    ...
    def _MacHandleKey(self, k_code, shift_down, alt_down, ctrl_down, cmd_down):
        ...
        if cmd_down and not ctrl_down:
            ...
            if k_code == wx.WXK_RIGHT:
                ...
                endsSpace = False
                if lpos > 0:
                    endsSpace = self.GetCharAt(lpos).isspace()
    ...
    def AddBookmark(self, line=-1):
        ...
        """
        rval = self.AddMarker(Bookmark(), line)
...
#         ed_msg.PostMessage(ed_msg.EDMSG_UI_STC_BOOKMARK, mdata)
    ...
    def RemoveBookmark(self, line):
        ...
        """
        self.RemoveMarker(Bookmark(), line)
...
#         ed_msg.PostMessage(ed_msg.EDMSG_UI_STC_BOOKMARK, mdata)
    ...
    def RemoveAllBookmarks(self):
        ...
        """
        self.RemoveAllMarkers(Bookmark())
...
#         ed_msg.PostMessage(ed_msg.EDMSG_UI_STC_BOOKMARK, mdata)
    ...
    def ShowCommandBar(self):
        ...
        self.TopLevelParent.GetEditPane().ShowCommandControl(ID_COMMAND)

    def ShowFindBar(self):
        """Open the quick-find bar"""
        self.TopLevelParent.GetEditPane().ShowCommandControl(ID_QUICK_FIND)
    # END TODO
...
#         self.SetCaretWidth(_PGET('CARETWIDTH', default=1))
    ...
    def SetCaretPos(self, pos):
        ...
        try:
            ...
            logger.debug("[ed_stc][err] SetCaretPos - invalid position %s" % pos)
...
#            util.SetClipboardText(stxt, primary=True)
    ...
    def OnLoadProgress(self, evt):
        ...
        if evt.GetState() == FL_STATE_READING:
            if evt.HasText():
...
#                sb.ProcessPendingEvents()
        ...
        elif evt.GetState() == FL_STATE_END:
            self.SetReadOnly(False)
#             ed_msg.PostMessage(ed_msg.EDMSG_PROGRESS_STATE, (pid, 0, 0))
            ...
            if hasattr(parent, 'DoPostLoad'):
                parent.DoPostLoad()
        elif evt.GetState() == FL_STATE_START:
...
#             ed_msg.PostMessage(ed_msg.EDMSG_PROGRESS_STATE, (pid, 0, self.File.GetSize()))
        ...
        elif evt.GetState() == FL_STATE_ABORTED:
            self.SetReadOnly(False)
            self.ClearAll()
...
#             ed_msg.PostMessage(ed_msg.EDMSG_UI_STC_MARGIN_CLICK, msgdata=data)
            if not data.get('handled', False):
                ...
                if Bookmark().IsSet(self, line_clicked):
    ...
    def ControlDispatch(self, evt):
        ...
        e_map = { ID_COPY  : self.Copy, ID_CUT  : self.Cut,
                  ...
                  ID_REDO  : self.Redo, ID_INDENT : self.Tab,
                  ID_REVERT_FILE : self.RevertToSaved,
                  ID_CUT_LINE : self.LineCut,
                  ID_DELETE_LINE : self.LineDelete,
                  ID_COLUMN_MODE : self.ToggleColumnMode,
                  ID_COPY_LINE : self.LineCopy,
                  ID_DUP_LINE : self.LineDuplicate,
                  ID_BRACKETHL : self.ToggleBracketHL,
                  ID_SYNTAX : self.SyntaxOnOff,
                  ID_UNINDENT : self.BackTab,
                  ID_TRANSPOSE : self.LineTranspose,
                  ID_LINE_MOVE_UP : self.LineMoveUp,
                  ID_LINE_MOVE_DOWN : self.LineMoveDown,
                  ID_SELECTALL: self.SelectAll,
                  ID_FOLDING : self.FoldingOnOff,
                  ID_SHOW_LN : self.ToggleLineNumbers,
                  ID_TOGGLECOMMENT : self.ToggleComment,
                  ID_AUTOINDENT : self.ToggleAutoIndent,
                  ID_LINE_AFTER : self.AddLine,
                  ID_TOGGLE_FOLD : self.ToggleFold,
                  ID_TOGGLE_ALL_FOLDS : self.FoldAll,
                  ID_TRIM_WS : self.TrimWhitespace,
                  ID_MACRO_START : self.StartRecord,
                  ID_MACRO_STOP : self.StopRecord,
                  ID_MACRO_PLAY : self.PlayMacro,
                  ID_GOTO_MBRACE : self.GotoBraceMatch,
                  ID_SHOW_AUTOCOMP : self.DoAutoComplete,
                  ID_SHOW_CALLTIP : self.DoCallTip
        }

        e_idmap = { ID_ZOOM_OUT : self.DoZoom,
                    ID_ZOOM_IN  : self.DoZoom,
                    ID_ZOOM_NORMAL : self.DoZoom,
                    ID_EOL_MAC  : self.ConvertLineMode,
                    ID_EOL_UNIX : self.ConvertLineMode,
                    ID_EOL_WIN  : self.ConvertLineMode,
                    ID_SPACE_TO_TAB : self.ConvertWhitespace,
                    ID_TAB_TO_SPACE : self.ConvertWhitespace,
                    ID_NEXT_MARK : self.Bookmark,
                    ID_PRE_MARK  : self.Bookmark,
                    ID_ADD_BM    : self.Bookmark,
                    ID_DEL_ALL_BM : self.Bookmark}
        ...
        elif e_id in SYNTAX_IDS:
            f_ext = GetExtFromId(e_id)
            logger.debug("[ed_stc][evt] Manually Setting Lexer to %s" % str(f_ext))
        ...
        elif e_id in [ID_TO_UPPER, ID_TO_LOWER]:
            self.ConvertCase(e_id == ID_TO_UPPER)
        elif e_id == ID_USE_SOFTTABS:
    ...
    def CheckEOL(self):
        ...
        eol = self.GetCharAt(self.GetLineEndPosition(0))
        if eol == u"\r":
            tmp = self.GetCharAt(self.GetLineEndPosition(0) + 1)
            if tmp == u"\n":
                eol += tmp
        ...
        if mixed or diff:
            if mixed:
                ...
                dlg = wx.Dialog(self.TopLevelParent, msg,
                                             _("Format EOL?"),
    ...
    def ConvertLineMode(self, mode_id):
        ...
        super().SetEOLMode(eol_map[mode_id])
    ...
    def GetEOLModeId(self):
        ...
        return eol_map.get(self.GetEOLMode(), ID_EOL_UNIX)
...
#         wx.PostEvent(self.TopLevelParent, evt)
    ...
    def FoldingOnOff(self, switch=None):
        ...
        if (switch is None and not self._config['folding']) or switch:
            logger.debug("[ed_stc][evt] Code Folding Turned On")
            self._config['folding'] = True
            self.SetMarginWidth(FOLD_MARGIN, 12)
            self.SetProperty("fold", "1")
        else:
            logger.debug("[ed_stc][evt] Code Folding Turned Off")
    ...
    def SyntaxOnOff(self, switch=None):
        ...
        if (switch is None and not self._config['highlight']) or switch:
            logger.debug("[ed_stc][evt] Syntax Highlighting Turned On")
            self._config['highlight'] = True
            self.FindLexer()
        else:
            logger.debug("[ed_stc][evt] Syntax Highlighting Turned Off")
...
#                    self.Expand(line_num, True)
    ...
    def ToggleBracketHL(self, switch=None):
        ...
        if (switch is None and not self._config['brackethl']) or switch:
            logger.debug("[ed_stc][evt] Bracket Highlighting Turned On")
            self._config['brackethl'] = True
            # Make sure to highlight a brace if next to on when turning it on
            self.DoBraceHighlight()
        else:
            logger.debug("[ed_stc][evt] Bracket Highlighting Turned Off")
    ...
    def ReloadFile(self):
        ...
        if os.path.exists(cfile):
            try:
                ...
                logger.debug("[ed_stc][err] Failed to Reload %s" % cfile)
                return False
            else:
                self.GotoPos(cpos)
                context = self.TopLevelParent.Id
#                 ed_msg.PostMessage(ed_msg.EDMSG_FILE_OPENED, self.GetFileName(), context)
            ...
            logger.debug("[ed_stc][err] %s does not exists, cant reload." % cfile)
